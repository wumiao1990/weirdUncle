...
namespace BaseTouchSystem
{
    ...
    public class EasyTouch : MonoBehaviour
    {
        ...
        //UGUI
        public bool enabledUGUIMode = false; // True = no events are send when touch is hover an ugui elements 
        private List<RaycastResult> uiRaycastResultCache = new List<RaycastResult>();
        private EventSystem uiEventSystem;
        private PointerEventData uguiEventData;
        ...
        void UpdateTouches(bool realTouch, int touchCount)
        {
            ...
            {
                ResetTouches();
                for (var i = 0; i < touchCount && i < fingers.Length; ++i)
                {
        ...
        private bool IsTouchHoverNGui(int touchIndex)
        {
            ...
            if (enabledUGUIMode)
            {
                returnValue = IsScreenPositionOverUI(fingers[touchIndex].position);
            }
        ...
        {
            var tmpSystem = EventSystem.current;
            if (tmpSystem != null)
            {
                if (uiEventSystem != tmpSystem)
                {
                    uiEventSystem = tmpSystem;
                    uguiEventData = new PointerEventData(uiEventSystem);
                }
                if (uguiEventData == null)
                {
                    uguiEventData = new PointerEventData(uiEventSystem);
                }

                uguiEventData.position = position;

                uiEventSystem.RaycastAll(uguiEventData, uiRaycastResultCache);
