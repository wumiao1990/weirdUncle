...
namespace Chronos
{
	public class RigidbodyTimeline2D : RigidbodyTimeline<Rigidbody2D, RigidbodyTimeline2D.Snapshot>
	{
		...
		{
			if (bodyIsManual && !isManual) // Chronos-manual, not user-manual
			{
			...
			if (gravityScale > 0 && !bodyIsManual && timeline.timeScale > 0)
			{
		...
		{
			get { return component.bodyType == RigidbodyType2D.Kinematic; }
			set { component.bodyType = value ? RigidbodyType2D.Kinematic : bodyType; }
		}
